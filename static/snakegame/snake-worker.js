...
function findpath_bfs() {
    ...
    var queue = new Array();
    var path = new Array(config.grid_size);
    for (var i = 0; i < config.grid_size; i++) {
        path[i] = new Array(config.grid_size);
    }
    //initialize path values to 0
    for (var i = 0; i < config.grid_size; i++) {
        for (var j = 0; j < config.grid_size; j++) {
            path[i][j] = 0;
        }
    ...
    while (queue.length != 0) {
        ...
        path[n.point.x][n.point.y] = 1;

        // Add adjacent nodes to queue to be processed.
        if (path[n.point.x][n.point.y - 1] == 0 && (squares[n.point.x][n.point.y - 1] == 0 || squares[n.point.x][n.point.y - 1] == 2))
            n.children.unshift(new Node(n, new Point(n.point.x, n.point.y - 1), new Array()));
        if (path[n.point.x + 1][n.point.y] == 0 && (squares[n.point.x + 1][n.point.y] == 0 || squares[n.point.x + 1][n.point.y] == 2))
...
function findpath_a(search_type) {
    ...
    // Adding our starting point to Open List
    queue.push(new Node(null, snake[0], new Array(), 0, heuristic_estimate(snake[0], food, search_type)));
    // Loop while queue contains some data.
    while (queue.length != 0) {
        ...
        if (path[n.point.x - 1][n.point.y] == 0 && (squares[n.point.x - 1][n.point.y] == 0 || squares[n.point.x - 1][n.point.y] == 2))
            n.children.unshift(new Node(n, new Point(n.point.x - 1, n.point.y), new Array(), n.g_score + 1, heuristic_estimate(new Point(n.point.x - 1, n.point.y), food, search_type)));
        for (var i = 0; i < n.children.length; i++) {
            var index = in_queue(queue, n.children[i]);
            if (index < 0) {
                //node not in queue, add it.
                queue.push(n.children[i]);
            } else {
                //found a node in queue that we already found earlier. Check if this is a better route
                if (n.children[i].f_score < queue[index].f_score) {
                    //better route, use this one instead.
                    //set the new parent for all the old child nodes
                    for (var j = 0; j < queue[index].children.length; j++) {
                        queue[index].children[j].parent = n.children[i];
                    }
                    //give the children to the new parent
                    n.children[i].children = queue[index].children;
                    //remove the old node from queue
...
//check is aNode is in queue. If a match is found, return index, -1 if no match
function in_queue(queue, aNode) {
    for (var i = 0; i < queue.length; i++) {
        if (queue[i].point.x == aNode.point.x && queue[i].point.y == aNode.point.y)
            return i;
    }
    return -1;
}
